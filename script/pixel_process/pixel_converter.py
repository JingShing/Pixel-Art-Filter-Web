...
def get_file_name_and_format(img_path):
    img_path.replace("\\", "/")
    img_file_name = re.split("/", img_path)[-1]
    img_file_format = img_file_name.split('.')[-1]
    return img_file_name, img_file_format
...
def convert(img_path, set_dict):
    ...
    colors = None
    img_file_name, img_file_format = get_file_name_and_format(img_path)
    if img_file_format in ['gif', 'GIF']:
        ...
        img_res, colors = transform(img_path, set_dict)
        img_path = img_path.split(img_file_name)[0]
    # cv2.imshow('show', img_res)
    return img_res, colors
